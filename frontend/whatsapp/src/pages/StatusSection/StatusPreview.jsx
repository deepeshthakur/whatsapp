...
const StatusPreview = ({
  ...
  const currentStatus = contact?.statues[currentIndex];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      current += 2; // increase progress by 2 percent by every 100 ms 50 steps = 5 second
      setProgress(current);
      if (current >= 100) {
  ...
  const handleViwersToggle = () => {
    setShowViewers(!showViewers);
  };

  const handleDeleteStaus = () => {
    if (onDelete && currentStatus?.id) {
      onDelete(currentStatus.id);
    }
    if (contact.statues.length === 1) {
      onclose();
    } else {
      onPrev();
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          {currentIndex < contact.statues.length - 1 && (
            <button
              onClick={onNext}
